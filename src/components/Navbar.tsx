...
const Nav: React.FC<NavProps> = ({ logo, links, onButtonClick, buttonText }) => {
  ...
  return (
    <nav className="z-20 flex flex-wrap items-center justify-between p-4 bg-white navbar" id="navbar">
        ...
        <div className="relative flex justify-center">
          <div className="relative">
            <input
              type="text"
              placeholder="Search..."
              className="py-2 pl-10 pr-4 text-sm text-gray-700 bg-gray-100 rounded-full focus:outline-none focus:bg-white focus:shadow-outline"
            />
      ...
      <div className="block lg:hidden" id="mobile-menu">
        ...
        >
          {isMenuOpen ? (
            <svg className="w-3 h-3 fill-current menu-close" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
              <title>Close</title>
            ...
            <svg className="w-3 h-3 fill-current menu-open" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
              <title>Menu</title>
      ...
      <div className={`w-full block lg:flex gap-x-5 lg:items-center lg:w-auto ${isMenuOpen ? "" : "hidden"}`} id="js-menu">
        <div className="grid text-sm gap-y-5 md:flex gap-x-5 lg:flex-grow">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className={`nav-links links ${isActive(link.href)}`} // Apply active styling
          ...
          {buttonText && (
            <button
              className="p-1 px-2 myButton text-white capitalize font-medium bg-[#fdb740] rounded-md"
